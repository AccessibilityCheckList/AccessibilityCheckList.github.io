    ...
    contents: [
      ...
      },
      {
        ...
        items: [
          ...
          {
            head: '意図しないフォーカス移動により誤入力を促していないか？',
            body: `
            マウスを利用するユーザーには便利だが、キーボードで操作するユーザーは意図しないフォーカスの移動で誤入力をしてしまう可能性がある。
            `,
            isOpen: false,
            isChecked: false,
          },
          {
            head: 'エラーを解決できないフォームになっていないか？',
            body: `
            エラー発生時にエラーのメッセージが見つからず、何が起きたかわからない設計になっていないか？エラーのメッセージはエラー内容にふさわしいか？
            ユーザーがエラー個所を特定できるデザインになっているかを考慮し設計する必要がある。可能な限り、フォームを送信前にエラーをユーザーに伝える必要がある。
            `,
            isOpen: false,
            isChecked: false,
          },
          {
            head: '確認訂正ができるフォームになっているか？',
            body: `
            フォーム送信前に再度入力内容を確認できるようになっているか？送信後のフォームの内容を訂正できる設計にする必要がある。
            `,
            isOpen: false,
            isChecked: false,
          },
          {
            head: '「戻る」が使えないフォームになっていないか？',
            body: `
            戻ると入力内容が消えている、戻って再送信するとエラーになる、そもそも戻るを禁止しているなどユーザーに戻るを使わせないフォームになっていないか？
            入力欄はautocomplete="off"にせず、可能な限り戻る遷移時に入力フォームを復元するようにする。
            システム側で厳密な画面遷移制御を行わず、戻って再送信された場合でも、多重送信を検出して適切に処理するようにする。
            `,
            isOpen: false,
            isChecked: false,
          },
          {
            head: '短時間でセッションが切れてしまうフォームになっていないか？',
            body: `
            入力に時間のかかるユーザーがいることを想定し、セッションの有効期限を長く設定する必要がある。また再ログイン後続きから作業ができるようにする。
            `,
            isOpen: false,
            isChecked: false,
          },
      ...
      },
      {
        ...
        items: [
          {
            head: 'ブラウザのページタイトルは適切か？',
            body: `
            ほかの類似ページと差別化できているタイトルになっているか？長すぎて切れていないか？コンテンツの内容とマッチしているか確認する必要がある。
            `,
            isOpen: false,
            isChecked: false,
          },
          {
            head: '適切な見出しがあるか？',
            body: `
            コンテンツが長文にならないよう、適切に見出しをいれているか？見出しの内容からコンテンツの把握ができるか？
            見出しと階層が正しく紐づいているかを確認する必要がある。
            `,
            isOpen: false,
            isChecked: false,
          },
          {
            head: 'わかりにくい言葉を使っていないか？',
            body: `
            読めない言葉（外国語、略語、難読後）を使っていないか？専門用語や一般的でない業界用語などを利用していないか確認する必要がある。
            `,
            isOpen: false,
            isChecked: false,
          },
          {
            head: '複雑な表で理解が難しくなっていないか？',
            body: `
            表題やキャプションはあるか？行や列に見出しがあるか？
            スクリーンリーダーのユーザーを考慮する場合、セルの結合や複数の表を結合すると理解できない可能性がある。
            `,
            isOpen: false,
            isChecked: false,
          },
          {
            head: '画像を含むコンテンツが理解できるようになっているか？',
            body: `
          　スクリーンリーダーユーザーなど画像を見ることができない環境ではコンテンツを全く理解できない。
            画像の代替テキスト(alt)が不適切な説明になっていないか？を確認する必要がある。
            `,
            isOpen: false,
            isChecked: false,
          },
          {
            head: 'リンク先がわかるようになっているか？',
            body: `
          　グラフィックの中のリンクなど、ラベルがなければリンク先がわからない可能性がある。
            "こちら"など文脈がなければリンク先を想像できないものになっていないかを確認する必要がある。 
            `,
            isOpen: false,
            isChecked: false,
          },
      ...
      },
      {
        ...
        items: [
          {
            head: '視覚に頼った情報の伝え方になっていないか？',
            body: `
          　色や配置、形や大きさを知るすべがないユーザーには情報を伝えることができない。
            視覚的な特徴に依存した表現をさけるようにする必要がある。
            `,
            isOpen: false,
            isChecked: false,
          },
          {
            head: 'コントラスト比に問題はないか？',
            body: `
          　文字が画像や背景に溶け込んでいないか？画像や図形ははっきりとしているか？
            入力フォームの色が背景と同化していないか？など、ユーザーに分かりやすい配色になっているか
            検討する必要がある。
            `,
            isOpen: false,
            isChecked: false,
          ...
          {
            head: 'タッチ(クリック)可能な場所が明示できているか？',
            body: `
          　リンクはリンクの外観になっているか？リンクではない個所がリンクに見えないか？訪問済みリンクと未訪問のリンクは色が別になっているか？
          　ボタンはボタンに見えるか？ボタンでないものがボタンに見えていないかなど検討する必要がある。
            `,
            isOpen: false,
            isChecked: false,
          },
          {
            head: 'テキストブロックが読みづらくなっていないか？',
            body: `
          　行間はつまり過ぎていないか？段落と行間は適度な幅になっているか？1行が長すぎないか？など検討する必要がある。
            `,
            isOpen: false,
            isChecked: false,
          },
          {
            head: '文字を画像で表現していないか？',
            body: `
          　文字を画像で表現すると、拡大ができず折り返しもできない。またスクリーンリーダーやハイコントラストモードに対応できないため、
          　可能な限り、CSSやWebフォント、SVGで対応する必要がある。
            `,
            isOpen: false,
            isChecked: false,
          },
          {
            head: 'フォーカス位置は確認できるか？',
            body: `
          　キーボードユーザーはボタンやテキストフィールドなどにフォーカスが当たっていることで選択している個所を特定している。
          　フォーカス位置がわかるように設定する必要がある。
            `,
            isOpen: false,
            isChecked: false,
          },
          {
            head: '操作対象が小さすぎないか？',
            body: `
          　マウスやキーボード以外での操作(指、ポインターなど)の場合、操作対象が小さすぎるとユーザーがうまく操作できない可能性がある。
          　ユーザーが操作しやすい適切なサイズにする必要がある。
            `,
            isOpen: false,
            isChecked: false,
          },
          {
            head: 'スタイルに一貫性はあるか？',
            body: `
          　同じ見た目で意味が異なるスタイルを適応していないか？逆に同じ意味合いで異なるスタイルを適応していないか？
          　配置に関しても一貫性を保ち、ユーザーの誤操作を促さないようにする必要がある。
            `,
            isOpen: false,
            isChecked: false,
          },
          {
            head: '急な閃光を利用していないか？',
            body: `
          　予告なく閃光を利用するとユーザーは発作を起こす可能性がある。あらかじめ閃光が発生する場合は注意を促す必要がある。
            `,
            isOpen: false,
            isChecked: false,
          },
          {
            head: '拡大に対応できているか？',
            body: `
          　表示を拡大すると見れなくなるコンテンツがあったり、そもそも拡大を許さないような設計になっていないか確認する必要がある。
            `,
            isOpen: false,
            isChecked: false,
          },
      ...
      },
      {
        ...
        items: [
          {
            head: '文法ミスはないか？',
            body: 'HTMLの閉じタグを忘れたり、同じIDを１画面内で複数回利用したり誤った使い方をしていないか確認する必要がある。',
            isOpen: false,
            isChecked: false,
          },
          {
            head: 'セマンティクスが不適切になっていないか？',
            body: 'HTMLのタグは適切に利用できているか？不要なメタ情報を載せていないか？本来のUIの意図に応じた実装をする必要がある。',
            isOpen: false,
            isChecked: false,
          },
          {
            head: 'コンテンツがCSSに依存していないか？',
            body: '見た目とHTMLが食い違い、意味が伝わらなくなっていないか確認する必要がある。',
            isOpen: false,
            isChecked: false,
          },
          {
            head: '代替テキストの実装が不適切になっていないか？',
            body: 'alt属性は設定されているか？不要に代替テキストを指定していないかなど確認する必要がある。',
            isOpen: false,
            isChecked: false,
          },
          {
            head: 'ラベルが正しくマークアップされているか？',
            body: 'ラジオや入力テキストに適切にラベルを設定できているかを検討する必要がある。',
            isOpen: false,
            isChecked: false,
          },
          {
            head: 'スクリーンリーダーの対応は適切か？',
            body: '無効なテキストがよみあげられたり、過剰な機能を提供していないかなど検討する必要がある。',
            isOpen: false,
            isChecked: false,
          },
        ]
